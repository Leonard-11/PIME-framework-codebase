...
def carica_foundation_models():
    print("\n[SYSTEM] Loading Foundation Models on CPU...")
    ...
    t0 = time.time()
    modelli["chronos_bolt"] = ChronosBoltPipeline.from_pretrained("amazon/chronos-bolt-mini", device_map=device)
    print(f"     -> Done in {time.time() - t0:.2f} seconds")
    
    print(" 2/4 Loading MOIRAI_SMALL...")
    t0 = time.time()
    moirai_module = Moirai2Module.from_pretrained("Salesforce/moirai-2.0-R-small").to(device).eval()
    ...
    t0 = time.time()
    percorso_onnx = hf_hub_download(repo_id="NX-AI/TiRex", filename="tirex.onnx")
...
class AnomalyDashboard:
    def __init__(self, root):
        ...
        # Strutture Dati per Grafico Unico
        self.plot_tempi = []
